...
- `cjtfquaaqzpaustuniiy.supabase.co` — Used for two purposes, both of which require an external database unavailable in the Devvit KV store:
  ...
  2. **Query event logging (`log_query_event` RPC):** After each trigger, a structured operational record is written to Supabase containing: the triggering post's Reddit post ID, aggregate match counts (candidates, deleted, valid), whether a comment was posted, and per-match metadata (post ID, permalink, similarity score, and deletion status). No post titles, body text, usernames, or user account information are included. This logging is essential for maintaining database quality over time: it provides the signal needed to identify and purge stale or deleted posts from the vector index, ensuring the app continues to surface relevant and accessible content rather than links to removed threads. Without this record, the app would have no durable mechanism to track which indexed posts are still live, causing match quality to degrade as the dataset ages. Supabase is an approved cloud provider.
